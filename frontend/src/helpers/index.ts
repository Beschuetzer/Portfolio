...
export const attachProgressListener = (
  video: HTMLVideoElement,
  hasProgressEventListener: boolean,
...
export function checkForParentOfType(
  ...
  try {
    ...
    if (
      (clickedElement.parentNode as HTMLElement).localName.search(/html/i) !==
      -1
    )
      return false;
...
export const getPercentOfProgressBar = (
...
export const handleVideoProgress = (
  videoRef: RefObject<HTMLVideoElement> | Event,
  progressBarRef: RefObject<HTMLProgressElement>,
  e: Event
) => {
  const video =
    (videoRef as any).current || ((videoRef as any).target as HTMLVideoElement);
  if (!video) return;
...
export const keypressHandler = (e: KeyboardEvent) => {
...
export const scrollToSection = (
  sectionToScrollTo: HTMLElement | null,
  addedHeight?: number
) => {
  ...
  const addedHeightToUse =
    addedHeight || -DEFAULT_FONT_SIZE * (isMobile ? 4 : 100);
  const headerHeight = isMobile
    ? parseFloat(
        getComputedStyleCustom(HEADER_HEIGHT_CUSTOM_PROPERTY_NAME) || "0"
      )
    : 0;
