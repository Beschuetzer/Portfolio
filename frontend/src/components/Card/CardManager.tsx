...
interface CardManagerProps {
  children: ReactChildren,
	...
	bridgeCards: HTMLElement[],
	setBridgeCards: (cards: HTMLElement[]) => void,
	isCardVideoOpen: boolean,
	setLastSecondRowCardNumber: (value: number) => void,
...
const CardManager: React.FC<CardManagerProps> = ({
	...
	const memoizedCheckForChanges = useCallback(() => {
		...
		if (secondRowCardNumber !== lastSecondRowCardNumber) {
			if (secondRowCardNumber) setLastSecondRowCardNumber(secondRowCardNumber);
		}
			...
			for (let i = 0; i < bridgeCards.length; i++) {
				...
				if (isTopRow) {
					...
					if (isFirstInRow) {
						...
						transformOriginToUse = transformOriginOptions.topRight;
					...
					if (isFirstInRow)
						transformOriginToUse = transformOriginOptions.bottomLeft;
					else if (isLastInRow)
						transformOriginToUse = transformOriginOptions.bottomRight;
					...
					if (isFirstInRow) transformOriginToUse = transformOriginOptions.left;
						...
						transformOriginToUse = transformOriginOptions.right;
					else {
						const middleRow = Math.ceil(numberOfRows / 2);
						const cutoffIndex = middleRow * rowLength - 1;

						if (i <= cutoffIndex)
							transformOriginToUse = transformOriginOptions.top;
						else transformOriginToUse = transformOriginOptions.bottom;
					}
				}
