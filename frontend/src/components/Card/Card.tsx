...
export const Card: React.FC<CardProps> = ({
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      if (section.id.match(/feature/i)) {
        const title = section.querySelector(
          `.${BRIDGE_CLASSNAME}__section-title`
        );
  ...
  const centerCard = (
    ...
    // console.log("cardOriginalHeight =", cardOriginalHeight);
  ...
  const getGapAmount = (video: HTMLVideoElement) => {
    const featuresBridgeSectionTitles =
      getFeaturesBridgeSectionTitles() as HTMLElement;
  ...
  const handleCardClick = (e: MouseEventHandler<HTMLElement>) => {
    ...
    setTimeout(() => {
      ...
      openCard(
        ...
        bridgeBackdrop as HTMLElement,
        initialCardSize as ClientRect
      );
      scrollToSection(
        document.querySelector(
          `#${bridgeSectionNames[1].toLowerCase()}`
        ) as HTMLElement
      );
