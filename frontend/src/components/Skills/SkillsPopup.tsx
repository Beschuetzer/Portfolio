...
import {
  ...
  clickedSkillSelector,
  reposToDisplaySelector,
    ...
    for (let i = 0; i < repos?.length; i++) {
      ...
      for (let j = 0; j < repo.repositoryTopics.nodes?.length; j++) {
        ...
        if (
          clickedSkill &&
          node?.topic?.name ===
            replaceCharacters(clickedSkill?.trim(), [[" ", "-"]])
        ) {
  ...
  const getProjectContent = (repo: any, key: string, index: number) => {
    switch (key) {
      case "name":
        ...
        } else if (repo["name"].search(/autobid/i) !== -1) {
          return (
            <SkillsPopupName key={key} href={`/examples/autobid`} repo={repo} />
          );
        } else if (repo["homepageUrl"]) {
          return (
            <SkillsPopupName key={key} href={repo["homepageUrl"]} repo={repo} />
          );
        }
        ...
        return (
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: addSpaceAfterPunctuationMarks(repo[key]),
            }}
          ></div>
        );
      default:
  ...
  const renderProjects = () => {
    ...
    return [...reposToDisplay]
      .sort((a: any, b: any) => {
        ...
        return firstItemsDate > secondItemsDate
          ? -1
          : firstItemsDate < secondItemsDate
          ? 1
          : 0;
      })
      .map((repo: any) => {
        if (isMobile) {
          return (
            <article
              key={repo.name}
              className={`${SKILLS_CLASSNAME}-popup__table-repo`}
            >
              {keys.map((key, index) => {
    ...
    <div className={`${SKILLS_CLASSNAME}-popup__content`}>
      <div className={`${SKILLS_CLASSNAME}-popup__header`}>
        <span className={`${SKILLS_CLASSNAME}-popup__header-text`}>
          <span className={`${SKILLS_CLASSNAME}-popup__header-skill`}>
            {capitalize(clickedSkill)}
          </span>{" "}
          Projects:
        </span>
        <svg
          onClick={(e: any) => onCloseClick()}
          className={`${SKILLS_CLASSNAME}-popup__close`}
        >
          <use xlinkHref="/sprite.svg#icon-close"></use>
        </svg>
        <h5 className={`${SKILLS_CLASSNAME}-popup__hint`}>
