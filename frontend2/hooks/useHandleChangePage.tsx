...
import { replaceCharacters, scrollToSection } from '../helpers';
...
export const useHandleChangePage = (match: Match) => {
	...
	const adjustColors = useCallback(() => {
		...
		for (let i = 0; i < colorSuffixes.length; i++) {
			...
			const colorVarTarget = `${colorVarRoot}${colorVarSuffix !== "" ? `-${colorVarSuffix}` : ""
				}${colorVarNumber}`;
	...
	}, [adjustColors, adjustHeaderStyles, currentUrl]);

	//moving to the hash
	useEffect(() => {
		if (!!location.hash) {
			setTimeout(() => {
				let destination = replaceCharacters(location.hash, [['%20', '-']]);
				destination = replaceCharacters(destination, [[' ', '-']]);
